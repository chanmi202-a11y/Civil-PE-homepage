...
const { initSqlDB, getDB } = require('./db-compat');
...
function seedAdmin(db) {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, '관리자', '관리자', 'admin')
  `).run(email, bcrypt.hashSync(pw, 10));
...
function seedSampleData(db) {
  if (db.prepare('SELECT COUNT(*) AS c FROM questions').get().c > 0) return;
  ...
  [
    ['토공사',   '🏗️','c1','(예) 흙막이 공법 완전 정복 — 엄지말뚝·CIP·SCW 비교',      '흙막이 공법의 종류와 각 공법의 특징, 적용 조건을 체계적으로 정리했습니다.',    '<h2>1. 흙막이 공법이란?</h2><p>흙막이 공법은 지반 굴착 시 인접 지반의 붕괴를 방지하기 위해 설치하는 가시설 구조물입니다.</p><h2>2. 공법 종류</h2><ul><li><strong>엄지말뚝+토류판:</strong> 가장 경제적, 차수성 없음</li><li><strong>CIP:</strong> 현장타설 콘크리트, 차수성 양호</li><li><strong>SCW:</strong> 시멘트 벽체, 차수성 우수</li><li><strong>Top-down:</strong> 대심도 굴착, 공기 단축</li></ul><div class="tip">💡 시험 포인트: 각 공법의 차수성과 적용 지반 조건을 비교 정리하세요.</div>'],
    ['콘크리트','🧱','c2','(예) 콘크리트 배합설계 핵심 — W/C비와 강도의 관계',          '물-시멘트비와 압축강도의 관계, 슬럼프·크리프 메커니즘을 실무 중심으로 설명합니다.','<h2>1. 배합설계 목적</h2><p>소요 강도, 내구성, 워커빌리티를 동시에 만족시키는 배합을 결정하는 과정입니다.</p><h2>2. W/C비와 강도</h2><p>물-시멘트비(W/C)가 작을수록 압축강도는 커집니다. 일반적으로 W/C = 40~65% 범위에서 설계합니다.</p>'],
